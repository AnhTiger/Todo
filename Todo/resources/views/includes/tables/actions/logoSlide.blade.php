...
<div wire:key = "logoCard2acstrttioddngrsdddds{!!$row->id!!}"  class="user-avatar img-120-cycle unset-background unset-margin-right unset-margin {!!$loadingSetting->table_showhide_logo_style!!}" wire:key = "logoCard1actions{!!$row->id!!}">
    <a href="javascript:void(0);"  data-bs-toggle="modal" data-bs-target="#viewLogofffcfffCfardSlid567u5678e{{ $row->code }}{{ $row->id }}"  class="{!! $loadingSetting->table_link_click_button_style !!}">            
...
<div class="user-info">
    <h6>
        <a href="javascript:void(0);" onclick="Livewire.dispatch('loadingCommonInfo',{id : {{ $todo->id }}, custom:'{!!$custom!!}' })"  data-bs-toggle="modal" data-bs-target="#commonInfo" class="{!!$loadingSetting->table_link_click_button_style!!}">
                {!!$row->name!!}{!!identifiedUserBelong($row->id,$listUserPriority,$loadingSetting)!!}
        </a> 
    </h6>
    <span class="sub-text fw-normal">{!!$loadingSetting->icon_email!!}<a href='mailto:{!!$row->email!!}'>{!!$row->email!!}</a></span>
</div>
<div wire:key = "logoCar545664d2a5credtrtions{!!$row->id!!}"> 
    <div class="modal fade {!!$loadingSetting->modal_view_more_info_form_style!!}" wire:ignore.self role="dialog" id="viewLogofffcfffCfardSlid567u5678e{{ $row->code }}{{ $row->id }}" tabindex="-1" aria-hidden="true" data-bs-keyboard="false">
